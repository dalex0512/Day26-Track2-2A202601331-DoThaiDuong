...
Read them in this order: `gateway.py` (the implemented control plane),
`strategy.py` (routing/budget policy helpers), `guardrails.py` (active checks
every `ANSWER` should pass), `telemetry.py` (private diagnostics), and
`prompt.md` (the live model policy).
...
| **`strategy.py`** | Routing, mask, cache, replica and delegation helpers used by the gateway's policy. | The same `command` / `tool_call` events as `gateway.py`, plus the credit trajectory across all 10 rounds. | `wasteful` **3** · `protocol_misuse` **6** · `stale_read` **8** |
| **`guardrails.py`** | Grounding, bilingual prompt-injection detection, privacy redaction, source-backed arithmetic checks, and abstention. | The `answer` L1 event (`text`, `cited_anchors`) handed to the opponent's prosecutor. | `ungrounded` **5** · `fabricated_citation` **8** · `hallucination` **7** · `guardrail_breach` **8** · `privacy_leak` **8** · `unsupported_precision` **4** |
